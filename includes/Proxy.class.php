...
class Proxy
{
    ...
    protected function addBroken($p)
    {
        if(is_writable($this->proxyFile)){
            $brokenProxyFile = fopen(ROOT.'/'.$this->proxyFile, "a") or die("Không thể mở tệp proxy bị hỏng!");
            $txt = '';
            if(is_array($p))
            {
                foreach($p as $v)
    ...
    public function saveProxy($plist = [])
    {
        if(!$this->hasError())
        {
            if(is_array($plist))
            {
                ...
                {
                    $proxyFile = fopen(ROOT.'/'.$this->proxyFile, "w") or die("Không thể mở tệp proxy!");
                    $prxy = '';
                    foreach($plist as $p)
                    {
                ...
                {
                    $this->error = 'Tệp proxy không thể ghi được ! -> <b>' . $this->proxyFile . '</b>';
                }
            }
            else
            {
                $this->error = 'Đã cung cấp danh sách proxy không hợp lệ !';
            }
        }
        return false;
    ...
    protected function loadProxy($t = 'active')
    {
        ...
        if(file_exists(ROOT.'/'.$this->proxyFile))
        {
            ...
            {
                $this->error = 'Tệp proxy không đọc được ! -> <b>' . $this->proxyFile . '</b>';
            }
        }
        else
        {
            $this->error = 'Tệp proxy không tồn tại ! -> <b>' . $this->proxyFile . '</b>';
        }
    }
