...
class App
{
        ...
        if (isset($_GET['a']) && !empty($_GET['a']))
        {
            ...
            if (in_array($this->action, $this->actions))
            {
                ...
                else
                {
                    ...
                    die('Phương pháp này không tồn tại trong ứng dụng !');
                }
            }
            else
            {
    ...
    protected function dashboard()
    {
        ...
        $this->setTitle('Bảng điều khiển |  Ứng dụng GDplyr');
    ...
    {
        $this->setTitle('Hồ sơ quản trị viên | ' . APP_NAME . ' Ứng dụng');
        $user = new User($this->db);
        ...
        if (Helper::isPost())
        {
            ...
            if (empty($username))
            {
                $this->addAlert('tên người dùng là bắt buộc !', 'danger');
            }
            else
            {
            ...
            if (!empty($password))
            {
                ...
                {
                    $this->addAlert('Xác nhận mật khẩu là bắt buộc !', 'danger');
                }
                else
                {
                    if ($password != $confirmPassword)
                    {
                        $this->addAlert('Mật khẩu không khớp !', 'danger');
                    }
                }
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] == 0)
            {
                ...
                {
                    $this->addAlert("Tải lên hình ảnh hồ sơ không thành công ! -> <b>{$imgDir}</b> thư mục không tồn tại . ", 'warning');
                }
                else
                {
                    if (!is_writable(ROOT . $imgDir))
                    {
                        $this->addAlert("Tải lên hình ảnh hồ sơ không thành công ! -> <b>{$imgDir}</b> thư mục không thể ghi được. ", 'warning');
                    }
                    else
                    {
                        ...
                        if (!$upname)
                        {
                            $this->addAlert("Tải lên hình ảnh hồ sơ không thành công. -> Định dạng tập tin không hợp lệ !", 'warning');
                        }
                        else
                        {
                            $img = $upname;
            ...
            if (!$this->hasAlerts())
            {
                ...
                if ($user->assign($data)->save())
                {
                    ...
                    $this->addAlert('Các thay đổi đã lưu thành công !', 'success');
                    $this->saveAlerts();
    ...
    protected function login()
    {
        ...
        if (Helper::isPost())
        {
            ...
            if (!empty($user))
            {
                ...
                {
                    $this->addAlert('Mật khẩu không hợp lệ!', 'danger');
                }
            }
            else
            {
                $this->addAlert('Tên sử dụng không hợp lệ !', 'danger');
            }
        ...
        if (!empty($action))
        {
            switch ($action)
            {
                ...
                case 'general':
                    ...
                    if (helper::isPost())
                    {
                        ...
                        if (isset($_FILES['logo']) && $_FILES['logo']['error'] == 0)
                        {
                            ...
                            if (!$upname)
                            {
                                $this->addAlert("Tải lên hình ảnh biểu trưng không thành công.  -> Định dạng tệp không hợp lệ !", 'warning');
                            }
                            else
                            {
                                $logo = $upname;
                        ...
                        if (isset($_FILES['favicon']) && $_FILES['favicon']['error'] == 0)
                        {
                            ...
                            if (!$upname)
                            {
                                $this->addAlert("Tải lên biểu tượng yêu thích không thành công.  -> Định dạng tệp không hợp lệ !", 'warning');
                            }
                            else
                            {
                                $favicon = $upname;
                        ...
                        $this->addAlert('Cài đặt đã lưu thành công !', 'success');
                        $this->saveAlerts();
                ...
                case 'backup':
                    ...
                    $this->setTitle('Sao lưu | ' . APP_NAME . ' Ứng dụng');
                ...
                case 'proxy':
                    ...
                    $this->setTitle('Cài đặt proxy | ' . APP_NAME . ' Ứng dụng');
                ...
                case 'gauth':
                    ...
                    $this->setTitle('Cài đặt GAuths | ' . APP_NAME . ' Ứng dụng');
                        ...
                        switch ($sAction)
                        {
                            ...
                            case 'edit':
                                ...
                                if (Helper::isPost())
                                {
                                    ...
                                    if (empty($clientId))
                                    {
                                        $this->addAlert('ID khách hàng là bắt buộc!', 'danger');
                                    }

                                    if (empty($clientSecret))
                                    {
                                        $this->addAlert('Client Secret khách hàng là bắt buộc!', 'danger');
                                    }

                                    if (empty($refreshToken))
                                    {
                                        $this->addAlert('Làm mới Mã thông báo là bắt buộc !', 'danger');
                                    }
    ...
    protected function ads($action = '', $id = '')
    {
        ...
        $this->setTitle('Quảng cáo | ' . APP_NAME . ' Ứng dụng');
        ...
        if (Helper::isPost())
        {
            switch ($action)
            {
                ...
                case 'save-vast':
                    ...
                    if (empty($xml))
                    {
                        $this->addAlert('Tệp XML là bắt buộc! ',' Nguy hiểm');
                    }

                    if (empty($offset))
                    {
                        $this->addAlert('Chênh lệch là bắt buộc !', 'danger');
                    }

                    if (empty($type))
                    {
                        $this->addAlert('Loại quảng cáo là bắt buộc !', 'danger');
                    }
                    ...
                    if (!$this->hasAlerts())
                    {
                        ...
                        if (!$isEdit)
                        {
                            ...
                            if ($id)
                            {
                                $this->addAlert('Quảng cáo VAST được Lưu thành công !', 'success');
                            }
                            else
                            {
                                $this->addAlert('Đã xảy ra sự cố!', 'danger');
                            }
                        }
                        else
                        {
            ...
            switch ($_GET['type'])
            {
                ...
                case 'import-link':
                    ...
                    if (!empty($url) && Helper::isUrl($url))
                    {
                        ...
                        if ($type != 'Direct')
                        {
                            ...
                            if (!IS_DUPLICATE)
                            {
                                ...
                                if ($rep !== false)
                                {
                                    ...
                                    $err = 'Liên kết này đã tồn tại !&nbsp; <a href="' . $l . '" target="_blank class="alert-link">xem liên kết tồn tại</a>';
                                }
                            }
                        ...
                        {
                            $err = 'Định dạng liên kết không được hỗ trợ !';
                        }
    ...
    protected function servers($action = '', $id = '')
    {
        ...
        $this->setTitle('Máy chủ | ' . APP_NAME . ' Ứng dụng');
        $server = new Server($this->db, $this->config);
            ...
            default:
                if (Helper::isPost())
                {
                    ...
                    if (!empty($domain))
                    {
                        ...
                        {
                            $this->addAlert('URL miền không hợp lệ !', 'danger');
                        }
                    }
                    else
                    {
                        $this->addAlert('Tên miền là bắt buộc !', 'danger');
                    }

                    if (empty($name)) $name = 'Máy chủ của tôi';
    ...
    protected function getMainS($link)
    {
                ...
                if ($link->isExit($id))
                {
                    ...
                    if ($file['status'] != 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                die('<h1>Video không khả dụng !</h1>');
                            }
                        }
            ...
            if (!empty($action))
            {
                switch ($action)
                {
                    ...
                    case 'edit':
                        ...
                        $this->setTitle('Thêm / chỉnh sửa liên kết | ' . APP_NAME . ' Ứng dụng');
                        ...
                        if (Helper::isPost())
                        {
                            ...
                            if (empty($mainLink))
                            {
                                $this->addAlert('Liên kết chính là bắt buộc !', 'danger');
                            }
                            else
                            {
                                if (!Helper::isUrl($mainLink))
                                {
                                    $this->addAlert('URL không hợp lệ được cung cấp cho liên kết chính !', 'danger');
                                }
                                else
                                {
                                    if (!IS_DUPLICATE)
                                    {
                                        ...
                                        {
                                            $l = PROOT . '/links/edit/' . $rep['id'];
                                            $this->addAlert('Liên kết chính đã tồn tại !&nbsp; <a href="' . $l . '" class="alert-link">xem liên kết tồn tại</a>', 'danger');
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                $this->addAlert('URL không hợp lệ được cung cấp cho liên kết thay thế !', 'danger');
                            }
                            ...
                            if (!empty($slug))
                            {
                                if ($link->isExit($slug))
                                {
                                    $this->addAlert('Slug video đã tồn tại !', 'danger');
                                }
                            }
                            ...
                            if (!$this->hasAlerts())
                            {
                                ...
                                if (!$link->hasError())
                                {
                                    ...
                                    if (!$isEdit)
                                    {
                                        $this->addAlert('Liên kết đã được lưu thành công !', 'success');
                                        $this->saveAlerts();
                    ...
                    case 'all':
                        ...
                        $this->setTitle('Tất cả các liên kết | ' . APP_NAME . ' Ứng dụng');
        ...
        {
            $this->setTitle('Nhập số lượng lớn | ' . APP_NAME . ' Ứng dụng');
        ...
        protected function displayAlerts()
        {
            if ($this->hasAlerts('', true))
            {
                ...
                {
                    $alertHtml .= '<div class="alert alert-' . $k . '" role="alert"><b>Cảnh báo:&nbsp;</b>';
                    if (count($v) == 1)
                    {
                        $alertHtml .= $v[0];
                    }
        ...
        protected function display($template, $isBlank = false)
        {
            ...
            if (!file_exists(TEMPLATE . '/' . $template . '.php'))
            {
                ...
                die('Tập tin ' . TEMPLATE . '/' . $template . '.php không tìm thấy !');
            }
        ...
        {
            if (!file_exists(TEMPLATE . '/' . $template . '.php')) die('Tập tin ' . $template . ' không tồn tại !');
        ...
        protected function _404()
        {
            ...
            die('<h1>Lỗi 404 Không Tìm Được Trang !</h1>');
        }
        ...
        protected function _400()
        {
            ...
            die('<h1>400 Yêu Cầu Không Hợp Lệ !</h1>');
        }
